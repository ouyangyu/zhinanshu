...
$(document).ready(function() { 
    ...
    var loginCallback = function(i) {
        ...
        if(i.status==1){
            ...
            //动画效果 20141213  欧阳宇
            var newValue = 0;
            $('.input-username').fadeOut(1000);
            $('.input-password').fadeOut(1000);
            $('.action-submit').fadeOut(500);
            $('.progress').fadeIn(1000);
            $('.progress').css("display","block");
            var _intervalInput = setInterval(function(){
                $("#account_input").css('margin-left',newValue+"px");
                $("#pwd_input").css('margin-left','-'+newValue+'px');
                $('.logo-dong').css('margin-top','-'+newValue+'px');
                newValue = newValue + 5;
                if(newValue == 25) {
                    clearInterval(_intervalInput);
